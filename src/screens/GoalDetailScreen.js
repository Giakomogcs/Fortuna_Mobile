...
const GoalDetailScreen = ({ route }) => {
  ...
  useEffect(() => {
    const fetchGoalDetails = async () => {
      try {
        const response = await fetch(`https://fortuna-api.onrender.com/api/goals/${goalId}`, {
          method: 'GET',
          headers: {
  ...
  return (
    <ScrollView style={{ padding: 20 }}>
      ...
      <Button mode="contained" onPress={() => alert('Exibir plano de ação')} style={{ marginVertical: 10 }}>
        Ver Plano
      </Button>
      <Divider style={{ marginVertical: 10 }} />

      <List.Accordion title="Fonte de Renda Adicional" left={props => <List.Icon {...props} icon="currency-usd" />}>
      ...
      <List.Accordion title="Plano de Contingência" left={props => <List.Icon {...props} icon="alert-circle" />}>
      ...
      <List.Accordion title="Ajustes de Monitoramento" left={props => <List.Icon {...props} icon="chart-line" />}>
      ...
      <List.Accordion title="Dicas de Economia" left={props => <List.Icon {...props} icon="lightbulb" />}>
      ...
      <List.Accordion title="Recursos" left={props => <List.Icon {...props} icon="book" />}>
      ...
      <List.Accordion title="Etapas" left={props => <List.Icon {...props} icon="steps" />}>
        {goal.planning.steps.map((step, index) => (
          <View key={index}>
            <Text style={{ fontWeight: 'bold', marginTop: 10 }}>Etapa {step.step}: {step.description}</Text>
